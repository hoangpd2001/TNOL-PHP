...
$(document).ready(function () {
  ...
  function showListGroup(groups) {
    ...
    if (groups.length == 0) {
      ...
      groups.forEach((group, index) => {
        let btn_hide =
          group.hienthi == 1
            ? `<a class="dropdown-item btn-hide-group" data-id="${group.mahocphan}" href="javascript:void(0)"><i class="nav-main-link-icon si si-eye me-2 text-dark"></i> Ẩn nhóm</a>`
            : `<a class="dropdown-item btn-unhide-group" data-id="${group.mahocphan}" href="javascript:void(0)"><i class="nav-main-link-icon si si-action-undo me-2 text-dark"></i> Huỷ ẩn</a>`;
                            ...
                            <div class="block-options">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        ${btn_hide}
                                        <a class="dropdown-item btn-delete-group" data-id="${group.mahocphan}" href="javascript:void(0)">
                        ...
                        <div class="block-content bg-body-light text-center">
                            <h3 class="fs-4 mb-3">
                                <a href="javascript:void(0)" class="link-fx">${group.tenmonhoc}</a>
                                   ...
                                   <div class="push">
                                <span class="badge bg-info text-uppercase fw-bold py-2 px-3">Học phần</span>
                            </div>
                            <h5 class="text-muted mb-3" style="font-size:13px">Lớp ${group.tenlop} - ${group.tenkhoahoc}</h5>
                        ...
                        <div class="block-content block-content-full">
                            <div class="row g-sm">
                                <div class="col-12">
                                    ...
                                    aria-controls="offcanvasExample" data-id="${group.mahocphan}" data-index="${index}">
                ...
                <div class="d-md-flex justify-content-md-between align-items-md-center">
                  <div class="p-1 p-md-2">
                    <h3 class="h4 fw-bold mb-2">
                      <a href="./test/start/${test.made}?loainhom=hocphan&manhom=${
          test.manhom
                          ...
                          ${
                            status.includes("Đã thi")
                              ? test.diemthi == null ||
                                ...
                                ? " - Điểm: Không công bố"
                                : ` - Điểm: ${parseFloat(test.diemthi).toFixed(
                                    2
                                  )}`
                              : ""
                          }
  ...
  function loadDataTest(manhom) {
    $.ajax({
      type: "post",
      url: "./test/getTestsModuleWithUserResult",
      data: {
        manhom: manhom,
      },
      dataType: "json",
  ...
  $("#form-search-group").on("input", function () {
    ...
    let result = groups.filter(
      (item) =>
        item.tennhom.toLowerCase().includes(content) ||
        item.hoten.toLowerCase().includes(content)
    );
