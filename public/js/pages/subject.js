      ...
      Dashmix.helpers("jq-validation"),
        jQuery(".form-add-subject").validate({
          ...
          messages: {
            ...
            tenkhoa: {
              required: "Vui lòng chọn tên khoa",
            },
          },
        });
    }
...
function showData(subjects) {
  ...
  subjects.forEach((subject) => {
    let tenkhoa = facultyMap[subject.makhoa] || "Không xác định";
    html += `<tr tid="${subject.mamonhoc}">
              ...
              <td class="d-none d-sm-table-cell text-center fs-sm">${tenkhoa}</td>
  ...
  return new Promise((resolve, reject) => {
    $.get(
      ...
      function (data) {
        ...
        $("#main-page-khoa").html(html);
        $("#tenkhoa").html(html); // luôn gán để form có dữ liệu
        resolve(data);
      },
      "json"
    ).fail(reject);
...
$(document).ready(async function () {
  ...
  $("#modal-add-subject").on("shown.bs.modal", function () {
    $("#tenkhoa").select2({
      dropdownParent: $("#modal-add-subject"),
      width: "100%",
