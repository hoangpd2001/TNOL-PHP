...
$test = $data["Test"];
$start = strtotime($test["thoigianbatdau"]);

if ($test["trangthai"] == 1) {
    // Nếu trạng thái là 1, thời gian kết thúc = thời gian bắt đầu + thoigianthi (đơn vị phút)
    $duration = intval($test["thoigianthi"]); // phút
    $end = strtotime("+{$duration} minutes", $start);
} elseif ($test["trangthai"] == -1) {
    // Nếu trạng thái là -1, dùng thời gian kết thúc gốc
    $end = strtotime($test["thoigianketthuc"]);
} else {
    // Trạng thái khác, có thể xử lý mặc định hoặc báo lỗi
    $end = $start; // hoặc $end = null;
} ?>
    ...
    <div class="col-lg-6 col-md-12 bg-white p-4 rounded">
        ...
        <div class="exam-info mb-3">
            ...
            <div class="row mb-3">
                ...
                <div class="col-6 text-end">
                    <span><?php echo date("H:i d/m/Y", $end); ?></span>
                </div>
            </div>
        ...
        if (isset($data["Check"]['diemthi']) && $data["Check"]['diemthi'] != '') {
            ...
            echo "<button name='continue-test' id='continue-test'   data-test='" . htmlspecialchars(json_encode($data), ENT_QUOTES, 'UTF-8') . "'  class='btn btn-hero btn-info w-100 btn-st' role='button'>Tiếp tục thi <i class='fa fa-angle-right'></i></button>";
        } else {
            ...
            else echo "<button name='start-test' id='start-test'   data-test='" . htmlspecialchars(json_encode($data), ENT_QUOTES, 'UTF-8') . "'  class='btn btn-hero btn-info w-100 btn-st' role='button'>Bắt đầu thi <i class='fa fa-angle-right'></i></button>";
        }
        ?>
    </div>
</div>
